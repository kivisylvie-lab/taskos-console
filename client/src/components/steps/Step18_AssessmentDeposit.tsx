...
export function Step18_AssessmentDeposit() {
  ...
  return (
    <div className="card animate-fade-in-up">
      <h2 className="text-xl font-bold mb-1">Step 18: 评估与知识沉淀</h2>
      <p className="text-gray-500 text-sm mb-6">
        系统已完成使用后评估和知识库沉淀判断（规则 30）。这是流水线的最后一步。
      </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className="badge bg-blue-100 text-blue-700">{s.knowledgeType}</span>
                    {(s.tags || []).map((tag: string, ti: number) => (
                      <span key={ti} className="badge bg-blue-100 text-blue-700">{tag}</span>
                    ))}
                  </div>
      ...
      <div className="p-4 bg-green-50 border border-green-200 rounded-lg mb-4">
        <p className="text-green-700 font-medium">🎉 恭喜！任务已完成全部 18 步流程</p>
      ...
      <div className="text-center">
        <button onClick={goToPrevStep} className="btn-secondary mb-3">返回上一步</button>
        <p className="text-sm text-gray-500 mb-3">
          {selectedCards.size > 0
            ? `已选择 ${selectedCards.size} 个知识卡片待沉淀到知识库`
            : '未选择任何知识卡片'}
        </p>
        <p className="text-xs text-gray-400">
          💡 提示：你可以返回修改交付物，或刷新页面开始新任务。
        </p>
      </div>
    </div>
  );
