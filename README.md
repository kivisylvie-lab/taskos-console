...
KIVI AI TaskOS Console 是一个本地运行的 Web 应用，提供**两种工作模式**：
...
| | Step 09 | 本地能力扫描 | 能力与工具选择 |
| | Step 10 | GitHub搜索判断 | **多Agent/人工审批** |
| | Step 11 | 多Agent判断 | 交付物与解决包判断 |
| | Step 12 | 能力路由 | **风险与指标判断** |
| | Step 13 | 确认方案 | 用户确认执行方案 |
| **解决包交付** | Step 14 | 执行生成 | **解决包生成** |
| | Step 15 | 质量检查 | **客户使用说明生成** |
| | Step 16 | 生成交付物 | 汇报材料生成 |
| | Step 17 | 调用记录与汇报 | AI能力调用记录生成 |
| | Step 18 | 评估与沉淀 | **指标验证与沉淀** |
...
| 任务分诊 / 痛点拆解 | Prompt (AI API) | KIVIDAILYLIFE 规则 2/4 | 本地母库规则 | 一次性诊断，Prompt 够用 |
| 知识库检索 | Prompt (AI API) | KIVIDAILYLIFE 规则 30 | 本地母库规则 | 语义搜索，需要 AI 判断 |
| 交付物判断 / 业务动作 | Prompt (AI API) | KIVIDAILYLIFE 规则 26 | 本地母库规则 | 类型判断需 AI |
| AI 组合方案 | Prompt (AI API) | KIVIDAILYLIFE 规则 23/24/25 | 本地母库规则 | 能力类型选择 + 具体能力评分 |
| 多 Agent 审查 | SubAgent | 13 种 Agent 类型 | KIVIDAILYLIFE 规则 29 | 多专业角色独立判断 |
| 文件生成 | Prompt (AI API) | 交付物生成 | AI 直接生成 | 内容生成 |
| 项目打包 | Tool (自动化) | archiver | 项目内工具 | ZIP 打包 |

**未使用的能力类型及原因**：
- Skill：MVP 阶段未创建独立 Skill 文件，规则通过系统 Prompt 嵌入
- Hook：当前未启用自动触发 Hook，手动确认替代
- MCP：本地运行，未接入外部 MCP 服务（BS 模式中已识别 MCP 接入点）
- Plugin：MVP 阶段不做能力打包
...
- ✅ 完整的 18 步 UI 向导（双模式）
- ✅ 客户解决包模式 Demo（痛点拆解 → 解决包封装 → 指标验证）
- ✅ 20 场景本地模糊匹配引擎
- ✅ AI API 集成（多平台支持）
- ✅ 文件上传和 ZIP 下载
- ✅ SSE 流式生成进度
- ✅ Demo 模式（无需 API Key 体验完整流程）
- ⚠️ 多 Agent 实际并发执行（当前为 Demo 模拟）
- ⚠️ GitHub 外部能力搜索（未接入，仅做判断）
- ⚠️ MCP 真实连接（已识别接入点，未实际配置）
- ⚠️ 视频处理（不支持）
- ⚠️ 多用户认证（单用户本地运行）

## 免责声明

本项目为 MVP Demo，由 AI 辅助生成。部分功能为模拟状态，不代表真实商用能力。建议在生产使用前进行全面测试和代码审查。
